# -*- coding: utf-8 -*-
import os
import kbengine.base as kbe
from kbengine import debug


def onBaseAppReady(isBootstrap):
    """
    kbe method.
    baseapp已经准备好了
    @param isBootstrap: 是否为第一个启动的baseapp
    @type isBootstrap: BOOL
    """
    debug.INFO_MSG('onBaseAppReady: isBootstrap=%s, appID=%s, bootstrapGroupIndex=%s, bootstrapGlobalIndex=%s' % \
             (isBootstrap, os.getenv("KBE_COMPONENTID"), os.getenv("KBE_BOOTIDX_GROUP"),
              os.getenv("KBE_BOOTIDX_GLOBAL")))

    # baseapp就绪时，创建一个 Space_Test 空间实体
    if isBootstrap:
        kbe.createEntityAnywhere("Space_Test", {})
        kbe.createEntityAnywhere("Space1", {})


def onReadyForLogin(isBootstrap):
    """
    kbe method.
    如果返回值大于等于1.0则初始化全部完成, 否则返回准备的进度值0.0~1.0。
    在此可以确保脚本层全部初始化完成之后才开放登录。
    @param isBootstrap: 是否为第一个启动的baseapp
    @type isBootstrap: BOOL
    """
    return 1.0


def onReadyForShutDown():
    """
    kbe method.
    进程询问脚本层：我要shutdown了，脚本是否准备好了？
    如果返回True，则进程会进入shutdown的流程，其它值会使得进程在过一段时间后再次询问。
    用户可以在收到消息时进行脚本层的数据清理工作，以让脚本层的工作成果不会因为shutdown而丢失。
    """
    debug.INFO_MSG('onReadyForShutDown()')
    return True


def onBaseAppShutDown(state):
    """
    kbe method.
    这个baseapp被关闭前的回调函数
    @param state: 0 : 在断开所有客户端之前
                  1 : 在将所有entity写入数据库之前
                  2 : 所有entity被写入数据库之后
    @type state: int
    """
    debug.INFO_MSG('onBaseAppShutDown: state=%i' % state)


def onInit(isReload):
    """
    kbe method.
    当引擎启动后初始化完所有的脚本后这个接口被调用
    @param isReload: 是否是被重写加载脚本后触发的
    @type isReload: bool
    """
    debug.INFO_MSG('onInit::isReload:%s' % isReload)


def onFini():
    """
    kbe method.
    引擎正式关闭
    """
    debug.INFO_MSG('onFini()')


def onCellAppDeath(addr):
    """
    kbe method.
    某个cellapp死亡
    """
    WARNING_MSG('onCellAppDeath: %s' % (str(addr)))


def onGlobalData(key, value):
    """
    kbe method.
    globalData有改变
    """
    debug.DEBUG_MSG('onGlobalData: %s' % key)


def onGlobalDataDel(key):
    """
    kbe method.
    globalData有删除
    """
    debug.DEBUG_MSG('onDelGlobalData: %s' % key)


def onBaseAppData(key, value):
    """
    kbe method.
    baseAppData有改变
    """
    debug.DEBUG_MSG('onBaseAppData: %s' % key)


def onBaseAppDataDel(key):
    """
    kbe method.
    baseAppData有删除
    """
    debug.DEBUG_MSG('onBaseAppDataDel: %s' % key)


def onLoseChargeCB(ordersID, dbid, success, datas):
    """
    kbe method.
    有一个不明订单被处理， 可能是超时导致记录被billing
    清除， 而又收到第三方充值的处理回调
    """
    debug.DEBUG_MSG('onLoseChargeCB: ordersID=%s, dbid=%i, success=%i, datas=%s' % \
              (ordersID, dbid, success, datas))
